...
class Convolutional_Reduced_Rank_Regression(torch.nn.Module):
    def __init__(
        ...
        print(f"Any NaNs in B1 419: {torch.any(torch.isnan(self.B1_))}")
        print(f"Any NaNs in B1 420: {torch.any(torch.isnan(self.parameters()['B1_']))}")
    ...
    def init_params(self):
        ...
        print(f"Any NaNs in B1 444: {torch.any(torch.isnan(B1))}")
        B1, B2, bias = standardizer(B1, dim=1), standardizer(B2, dim=1), standardizer(bias, dim=0)
        print(f"Any NaNs in B1 446: {torch.any(torch.isnan(B1))}")
        ...
        print(f"Any NaNs in B1 453: {torch.any(torch.isnan(B1))}")
